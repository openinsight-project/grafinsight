...
def get_zip(version, target_filename):
    ...
    if exists:
        return target_filename
    url = 'https://s3-us-west-2.amazonaws.com/grafinsight-releases/release/grafinsight-{}.windows-amd64.zip'.format(version)
    #url = 'https://dl.grafinsight.com/enterprise/release/grafinsight-enterprise-{}.windows-amd64.zip'.format(version)
    filename = wget.download(url, out=target_filename, bar=wget.bar_thermometer)
    return filename
...
def detect_version(dist_path):
    ...
    fileList = glob.glob(dist_path + '/grafinsight*.windows-amd64.zip')
    print(fileList)
    if len(fileList) == 0:
        print('Skipping detection, no matches')
        return
    firstFile = fileList[0]
    p1 = re.search(r'grafinsight-(enterprise-)?(\d\.\d\.\d)-(.+)\.windows-amd64\.zip$', firstFile)
    p2 = re.search(r'grafinsight-(enterprise-)?(\d\.\d\.\d)\.windows-amd64\.zip$', firstFile)
    if p1:
    ...
    #    # grafinsight-enterprise-6.0.0-29b28127pre3.windows-amd64.zip
    #    # get files in directory matching pattern
    #    fileList = glob.glob(dist_path + '/enterprise-dist/grafinsight*.windows-amd64.zip')
    #    firstFile = fileList[0]
    #    p1 = re.search(r'grafinsight-enterprise-(\d\.\d\.\d)\.windows-amd64.zip$', firstFile)
    #    p2 = re.search(r'grafinsight-enterprise-(\d\.\d\.\d)-(.*)\.windows-amd64.zip$', firstFile)
    #    if p1:
...
def generate_service_wxs(env, grafinsight_version, scratch_file, target_dir, nssm_version='2.24'):
    template = env.get_template('common/grafinsight-service.wxs.j2')
    output = template.render(grafinsight_version=grafinsight_version, nssm_version=nssm_version)
    fh = open(scratch_file, 'w')
    fh.write(output)
    fh.close()
    shutil.copy2(scratch_file, target_dir)

def generate_firewall_wxs(env, grafinsight_version, scratch_file, target_dir):
    os.system("ls -al templates")
    template = env.get_template('common/grafinsight-firewall.wxs.j2')
    output = template.render(grafinsight_version=grafinsight_version)
    fh = open(scratch_file, 'w')
    fh.write(output)
    fh.close()
    shutil.copy2(scratch_file, target_dir)
