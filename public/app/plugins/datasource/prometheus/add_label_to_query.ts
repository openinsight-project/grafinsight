...
// We are creating 2 matching groups. (\$) is for the GrafInsight's variables such as ${__rate_s}. We want to ignore
...
export function addLabelToQuery(
  ...
  while (match) {
    ...
    // If we matched 1st group, we know it is GrafInsight's variable and we don't want to add labels
    if (match[1]) {
      parts.push(prefix);
      parts.push(match[0]);
    } else {
