...
describe('Test Datasource Query Editor', () => {
  ...
  it('should switch scenario and display its default values', async () => {
    ...
    await fireEvent.keyDown(select, { keyCode: 40 });
    await userEvent.click(screen.getByText('GrafInsight API'));
    expect(mockOnChange).toHaveBeenCalledWith(
      expect.objectContaining({ scenarioId: 'grafinsight_api', stringInput: 'datasources' })
    );
    rerender(
      <QueryEditor {...props} query={{ ...defaultQuery, scenarioId: 'grafinsight_api', stringInput: 'datasources' }} />
    );
    expect(await screen.findByText('GrafInsight API')).toBeInTheDocument();
