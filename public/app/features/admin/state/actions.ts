...
export function updateUserPermissions(userId: number, isGrafinsightAdmin: boolean): ThunkResult<void> {
  return async (dispatch) => {
    const payload = { isGrafinsightAdmin };
...
export function loadUserMapping(username: string): ThunkResult<void> {
  return async (dispatch) => {
    try {
      ...
      const { name, surname, email, login, isGrafinsightAdmin, isDisabled, roles, teams } = response;
      const userInfo: LdapUser = {
        info: { name, surname, email, login },
        permissions: { isGrafinsightAdmin, isDisabled },
        roles,
        teams,
      };
