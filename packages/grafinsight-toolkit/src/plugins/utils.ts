...
import fs from 'fs';
import { KeyValue } from '@grafinsight/data';
import { ExtensionSize, ZipFileInfo, GitLogInfo } from './types';

const md5File = require('md5-file');

export function getGrafInsightVersions(): KeyValue<string> {
  const dir = path.resolve(process.cwd(), 'node_modules', '@grafinsight');
