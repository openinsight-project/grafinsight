...
export async function signManifest(manifest: ManifestInfo): Promise<string> {
  ...
  if (!GrafInsight_API_KEY) {
    throw new Error('You must enter a GrafInsight_API_KEY to sign the plugin manifest');
  }

  const GrafInsight_COM_URL = process.env.GrafInsight_COM_URL || 'https://grafinsight.com/api';
  const url = GrafInsight_COM_URL + '/plugins/ci/sign';
