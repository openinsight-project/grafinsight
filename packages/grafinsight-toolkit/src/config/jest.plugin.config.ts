...
export const jestConfig = (baseDir: string = process.cwd()) => {
  ...
  if (deniedOverrides.length > 0) {
    console.error("\ngrafinsight-toolkit doesn't support following Jest options: ", deniedOverrides);
...
export const loadJestPluginConfig = (baseDir: string = process.cwd()) => {
  ...
  if (!fs.existsSync(cfgpath)) {
    const src = path.resolve(baseDir, 'node_modules/@grafinsight/toolkit/src/config/jest.plugin.config.local.js');
