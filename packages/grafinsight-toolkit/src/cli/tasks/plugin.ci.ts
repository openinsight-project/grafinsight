...
import { getPackageDetails, getGrafInsightVersions, readGitLog } from '../../plugins/utils';
 ...
 *  3. prepare grafinsight environment in: `~/ci/grafinsight-test-env`
 */
const packagePluginRunner: TaskRunner<PluginCIOptions> = async ({ signatureType, rootUrls }) => {
  ...
  const grafinsightEnvDir = path.resolve(ciDir, 'grafinsight-test-env');
  await execa('rimraf', [packagesDir, distDir, grafinsightEnvDir]);
  ...
  fs.mkdirSync(grafinsightEnvDir);
  ...
  console.log('Setup GrafInsight Environment');
  let p = path.resolve(grafinsightEnvDir, 'plugins', pluginInfo.id);
  ...
  p = path.resolve(grafinsightEnvDir, 'custom.ini');
  const customIniBody =
    `# Autogenerated by @grafinsight/toolkit \n` +
    `[paths] \n` +
    `plugins = ${path.resolve(grafinsightEnvDir, 'plugins')}\n` +
    `\n`; // empty line
...
const pluginReportRunner: TaskRunner<PluginCIOptions> = async ({ upload }) => {
  ...
  const report: PluginBuildReport = {
    ...
    grafinsightVersion: getGrafInsightVersions(),
  ...
  const GrafInsight_API_KEY = process.env.GrafInsight_API_KEY;
  if (!GrafInsight_API_KEY) {
    console.log('Enter a GrafInsight_API_KEY to upload the plugin report');
    return;
  }
  const url = `https://grafinsight.com/api/plugins/${report.plugin.id}/ci`;

  console.log('Sending report to:', url);
  const axios = require('axios');
  const info = await axios.post(url, report, {
    headers: { Authorization: 'Bearer ' + GrafInsight_API_KEY },
  });
  if (info.status === 200) {
