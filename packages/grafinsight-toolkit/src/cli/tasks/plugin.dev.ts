...
const yarnlink = () =>
  useSpinner('Linking local toolkit', async () => {
    try {
      ...
      await execa('yarn', ['remove', '@grafinsight/toolkit']);
    } catch (e) {
      console.log('\n', e.message, '\n');
    }
    await execa('yarn', ['link', '@grafinsight/toolkit']);
    ...
    console.log('Added dependencies required by local @grafinsight/toolkit.  Do not checkin this package.json!');
  });
