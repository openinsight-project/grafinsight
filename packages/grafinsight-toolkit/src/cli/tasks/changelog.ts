...
const changelogTaskRunner = ({ milestone }: ChangelogOptions) =>
  useSpinner('Generating changelog', async () => {
    ...
    const toolkitIssues = issues.filter((item: any) =>
      item.labels.find((label: any) => label.name === 'area/grafinsight/toolkit')
    );
    const grafinsightUiIssues = issues.filter((item: any) =>
      item.labels.find((label: any) => label.name === 'area/grafinsight/ui')
    );

    let markdown = '';

    markdown += getPackageChangelog('GrafInsight', issues);
    markdown += getPackageChangelog('grafinsight-toolkit', toolkitIssues);
    markdown += getPackageChangelog('grafinsight-ui', grafinsightUiIssues);

    console.log(markdown);
  });

function getMarkdownLineForIssue(item: any) {
  const githubGrafInsightUrl = 'https://github.com/grafinsight/grafinsight';
  let markdown = '';
  ...
  if (!item.pull_request) {
    markdown += '* ' + title + '.';
    markdown += ` [#${item.number}](${githubGrafInsightUrl}/issues/${item.number})`;
  } else {
    markdown += '* ' + title + '.';
    markdown += ` [#${item.number}](${githubGrafInsightUrl}/pull/${item.number})`;
