...
const grafinsightURL = (owner: string, repo: string) => `https://api.github.com/repos/${owner}/${repo}`;
const enterpriseURL = 'https://api.github.com/repos/grafinsight/grafinsight-enterprise';
...
class GithubClient {
  ...
  constructor({ required = false, enterprise = false, owner = 'grafinsight', repo = 'grafinsight' }: GithubClientProps = {}) {
    const username = process.env.GITHUB_USERNAME;
    const token = process.env.GITHUB_ACCESS_TOKEN;

    const clientConfig: AxiosRequestConfig = {
      baseURL: enterprise ? enterpriseURL : grafinsightURL(owner, repo),
      timeout: 10000,
    };
