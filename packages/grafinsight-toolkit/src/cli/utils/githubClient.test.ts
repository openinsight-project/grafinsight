...
describe('GithubClient', () => {
  ...
  describe('#client', () => {
    it('it should contain a grafinsight client', () => {
      // @ts-ignore
      ...
      expect(spy).toHaveBeenCalledWith({
        baseURL: 'https://api.github.com/repos/grafinsight/grafinsight',
        timeout: 10000,
      });
      expect(client).toEqual(fakeClient);
    });

    it('it should contain a grafinsight enterprise client', () => {
      // @ts-ignore
      const spy = jest.spyOn(GithubClient.prototype, 'createClient').mockImplementation(() => fakeClient);
      ...
      expect(spy).toHaveBeenCalledWith({
        baseURL: 'https://api.github.com/repos/grafinsight/grafinsight-enterprise',
        timeout: 10000,
      });
      expect(client).toEqual(fakeClient);
    });
