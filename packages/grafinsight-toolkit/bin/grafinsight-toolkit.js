...
const isLinkedMode = () => {
  ...
  if (path.basename(pwd) === 'grafinsight-toolkit') {
    return true;
  }

  try {
    const resolvedPath = path.resolve(`${__dirname}/../../../node_modules/@grafinsight/toolkit`);
...
const entrypoint = () => {
  ...
  // IF we have a toolkit directory AND linked grafinsight toolkit AND the toolkit dir is a symbolic lik
