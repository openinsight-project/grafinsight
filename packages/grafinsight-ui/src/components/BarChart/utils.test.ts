...
import { FieldConfig, FieldType, GrafInsightTheme, MutableDataFrame, VizOrientation } from '@grafinsight/data';
...
describe('GraphNG utils', () => {
  describe('preparePlotConfigBuilder', () => {
    ...
    it.each([VizOrientation.Auto, VizOrientation.Horizontal, VizOrientation.Vertical])('orientation', (v) => {
      expect(
        preparePlotConfigBuilder(frame!, { colors: { panelBg: '#000000' } } as GrafInsightTheme, {
          ...config,
          orientation: v,
        })
      ).toMatchSnapshot();
