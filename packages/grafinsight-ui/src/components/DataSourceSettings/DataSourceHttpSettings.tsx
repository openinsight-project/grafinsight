...
import { DataSourceSettings, SelectableValue } from '@grafinsight/data';
...
const HttpAccessHelp = () => (
  <div className="grafinsight-info-box m-t-2">
    ...
    <p>
      All requests will be made from the browser to GrafInsight backend/server which in turn will forward the requests to
      the data source and by that circumvent possible Cross-Origin Resource Sharing (CORS) requirements. The URL needs
      to be accessible from the grafinsight backend/server if you select this access mode.
    </p>
...
export const DataSourceHttpSettings: React.FC<HttpSettingsProps> = (props) => {
  ...
  switch (dataSourceConfig.access) {
    ...
    case 'proxy':
      urlTooltip = (
        <>
          Your access method is <em>Server</em>, this means the URL needs to be accessible from the grafinsight
          backend/server.
        </>
      );
      break;
        ...
        <div className="gf-form-group">
          ...
          {dataSourceConfig.access === 'proxy' && (
            <div className="gf-form">
              <InlineFormLabel
                width={11}
                tooltip="GrafInsight Proxy deletes forwarded cookies by default. Specify cookies by name that should be forwarded to the data source."
              >
                Whitelisted Cookies
              </InlineFormLabel>
              <TagsInput
