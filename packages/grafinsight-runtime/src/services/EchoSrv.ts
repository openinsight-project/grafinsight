...
export interface EchoMeta {
  ...
  sessionId: string;
  /**
   * The current users username used to login into GrafInsight e.g. email.
   */
  userLogin: string;
  /**
  ...
  userId: number;
  /**
   * True when user is logged in into GrafInsight.
   */
...
/**
 * Used during startup by GrafInsight to set the EchoSrv so it is available
...
/**
 * Used to register echo backends that will receive GrafInsight echo events during application
 * runtime.
 *
 * @public
 */
