...
const isOptionsGroupOpen = (name: string): any =>
  requireLocalStorage(`grafinsight.dashboard.editor.ui.optionGroup[Panel ${name}]`).then(({ defaultToClosed }: any) => {
    // @todo remove `wrap` when possible
...
const isOptionsOpen = (): any =>
  getLocalStorage('grafinsight.dashboard.editor.ui').then((data: any) => {
    if (data) {
      // @todo remove `wrap` when possible
