...
import { grafinsightDataFrameToArrowTable, arrowTableToDataFrame } from './ArrowDataFrame';
...
describe('Read/Write arrow Table to DataFrame', () => {
  test('should parse output with dataframe', () => {
    ...
    const table = grafinsightDataFrameToArrowTable(frame);
  ...
  test('should support duplicate field names', () => {
    ...
    const table = grafinsightDataFrameToArrowTable(frame, true);
    expect(table.length).toEqual(frame.length);
  ...
  test('Export arrow table names', () => {
    ...
    const t1 = grafinsightDataFrameToArrowTable(simple);
    const t2 = grafinsightDataFrameToArrowTable(simple, true);
