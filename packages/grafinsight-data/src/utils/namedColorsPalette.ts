...
let colorsMapTheme: GrafInsightTheme | undefined;
...
export function buildColorsMapForTheme(theme: GrafInsightTheme): Record<Color, string> {
  theme = theme ?? GrafInsightThemeType.Dark;
...
export function getColorForTheme(color: string, theme: GrafInsightTheme): string {
  if (!color) {
    return 'gray';
  }
 ...
 */
export function getColorFromHexRgbOrName(color: string, type?: GrafInsightThemeType): string {
  const themeType = type ?? GrafInsightThemeType.Dark;

  if (themeType === GrafInsightThemeType.Dark) {
    const darkTheme = ({
      type: themeType,
      colors: {
        panelBg: '#141619',
