import { GrafInsightConfig, RawTimeRange, ScopedVars } from '../types';
import { urlUtil } from './url';
import { textUtil } from '../text';

let grafinsightConfig: () => GrafInsightConfig;
...
const stripBaseFromUrl = (url: string): string => {
  const appSubUrl = grafinsightConfig ? grafinsightConfig().appSubUrl : '';
...
const assureBaseUrl = (url: string): string => {
  if (url.startsWith('/')) {
    return `${grafinsightConfig ? grafinsightConfig().appSubUrl : ''}${stripBaseFromUrl(url)}`;
  }
  return url;
};

interface LocationUtilDependencies {
  getConfig: () => GrafInsightConfig;
...
export const locationUtil = {
  ...
  processUrl: (url: string) => {
    return grafinsightConfig().disableSanitizeHtml ? url : textUtil.sanitizeUrl(url);
  },
};
