...
import { GrafInsightThemeType } from '../types/theme';
...
describe('colors', () => {
  ...
  describe('getColorFromHexRgbOrName', () => {
    ...
    it("returns correct variant's hex for known color if theme specified", () => {
      expect(getColorFromHexRgbOrName(SemiDarkBlue.name, GrafInsightThemeType.Light)).toBe(SemiDarkBlue.variants.light);
    });
