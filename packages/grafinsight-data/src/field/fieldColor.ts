import { FALLBACK_COLOR, Field, FieldColorModeId, GrafInsightTheme, Threshold } from '../types';
...
export interface FieldColorMode extends RegistryItem {
  getCalculator: (field: Field, theme: GrafInsightTheme) => FieldValueColorCalculator;
  colors?: string[];
...
export class FieldColorSchemeMode implements FieldColorMode {
  ...
  private getColors(theme: GrafInsightTheme) {
    if (this.colorCache) {
  ...
  getCalculator(field: Field, theme: GrafInsightTheme) {
 ...
 */
export function getFieldSeriesColor(field: Field, theme: GrafInsightTheme): ColorScaleValue {
...
function getFixedColor(field: Field, theme: GrafInsightTheme) {
  return () => {
