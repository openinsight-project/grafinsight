...
// StartGrafinsight starts a Grafinsight server.
// The server address is returned.
func StartGrafinsight(t *testing.T, grafDir, cfgPath string, sqlStore *sqlstore.SQLStore) string {
	t.Helper()
	...
	// Wait for Grafinsight to be ready
	...
	t.Logf("Grafinsight is listening on %s", addr)

	return addr
}

// SetUpDatabase sets up the Grafinsight database.
...
// CreateGrafDir creates the Grafinsight directory.
func CreateGrafDir(t *testing.T, opts ...GrafinsightOpts) (string, string) {
	t.Helper()
	...
	require.NoError(t, err)
	// Copy index template to index.html, since Grafinsight will try to use the latter
	err = fs.CopyFile(filepath.Join(rootDir, "public", "views", "index-template.html"),
		filepath.Join(viewsDir, "index.html"))
	require.NoError(t, err)
	// Copy error template to error.html, since Grafinsight will try to use the latter
