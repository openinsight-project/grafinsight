...
func TestFixIntervalFormat(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			target:   "aliasByNode(hitcount(averageSeries(app.grafinsight.*.dashboards.views.count), '1m'), 4)",
			expected: "aliasByNode(hitcount(averageSeries(app.grafinsight.*.dashboards.views.count), '1min'), 4)",
		},
		{
			name:     "should transform 1M to graphite unit (1mon) when used as interval string",
			target:   "aliasByNode(hitcount(averageSeries(app.grafinsight.*.dashboards.views.count), '1M'), 4)",
			expected: "aliasByNode(hitcount(averageSeries(app.grafinsight.*.dashboards.views.count), '1mon'), 4)",
		},
		{
			name:     "should not transform 1m when not used as interval string",
			target:   "app.grafinsight.*.dashboards.views.1m.count",
			expected: "app.grafinsight.*.dashboards.views.1m.count",
		},
		{
			name:     "should not transform 1M when not used as interval string",
			target:   "app.grafinsight.*.dashboards.views.1M.count",
