...
func TestAzureLogAnalyticsMacros(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			kql:      `CounterPath in ($__escapeMulti('\\grafinsight-vm\Network(eth0)\Total','\\grafinsight-vm\Network(eth1)\Total'))`,
			expected: `CounterPath in (@'\\grafinsight-vm\Network(eth0)\Total', @'\\grafinsight-vm\Network(eth1)\Total')`,
			Err:      require.NoError,
		},
		{
			...
			kql:      `$__escapeMulti('\\grafinsight-vm,\Network(eth0)\Total Bytes Received')`,
			expected: `@'\\grafinsight-vm,\Network(eth0)\Total Bytes Received'`,
			Err:      require.NoError,
		},
		{
