...
const ExporterName = "grafinsight"
...
var (
	...
	// StatsTotalAnnotations is a metric of total number of annotations stored in GrafInsight.
	StatsTotalAnnotations prometheus.Gauge

	// StatsTotalDashboardVersions is a metric of total number of dashboard versions stored in GrafInsight.
	StatsTotalDashboardVersions prometheus.Gauge

	// grafinsightBuildVersion is a metric with a constant '1' value labeled by version, revision, branch, and goversion from which GrafInsight was built
	grafinsightBuildVersion *prometheus.GaugeVec

	grafinsightPluginBuildInfoDesc *prometheus.GaugeVec
)
...
func init() {
	...
	grafinsightBuildVersion = prometheus.NewGaugeVec(prometheus.GaugeOpts{
		Name:      "build_info",
		Help:      "A metric with a constant '1' value labeled by version, revision, branch, and goversion from which GrafInsight was built",
		Namespace: ExporterName,
	}, []string{"version", "revision", "branch", "goversion", "edition"})

	grafinsightPluginBuildInfoDesc = prometheus.NewGaugeVec(prometheus.GaugeOpts{
		Name:      "plugin_build_info",
		Help:      "A metric with a constant '1' value labeled by pluginId, pluginType and version from which GrafInsight plugin was built",
		Namespace: ExporterName,
...
func SetBuildInformation(version, revision, branch string) {
	...
	grafinsightBuildVersion.WithLabelValues(version, revision, branch, runtime.Version(), edition).Set(1)
}
...
func SetEnvironmentInformation(labels map[string]string) error {
	...
	grafinsightEnvironmentInfo := prometheus.NewGauge(prometheus.GaugeOpts{
	...
	prometheus.MustRegister(grafinsightEnvironmentInfo)

	grafinsightEnvironmentInfo.Set(1)
	return nil
}

func SetPluginBuildInformation(pluginID, pluginType, version string) {
	grafinsightPluginBuildInfoDesc.WithLabelValues(pluginID, pluginType, version).Set(1)
}
...
func initMetricVars() {
	prometheus.MustRegister(
		...
		grafinsightBuildVersion,
		grafinsightPluginBuildInfoDesc,
