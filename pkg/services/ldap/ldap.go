...
// 1. "admin" user
// Bind the "admin" user (defined in Grafinsight config file) which has the search privileges
...
// 2. Single bind
// // If all the users meant to be used with Grafinsight have the ability to search in LDAP server
...
func (server *Server) Login(query *models.LoginUserQuery) (
	*models.ExternalUserInfo, error,
	...
	user := users[0]
	if err := server.validateGrafinsightUser(user); err != nil {
		return nil, err
	}
...
// validateGrafinsightUser validates user access.
// If there are no ldap group mappings access is true
// otherwise a single group must match
func (server *Server) validateGrafinsightUser(user *models.ExternalUserInfo) error {
...
// buildGrafinsightUser extracts info from UserInfo model to ExternalUserInfo
func (server *Server) buildGrafinsightUser(user *ldap.Entry) (*models.ExternalUserInfo, error) {
	...
	for _, group := range server.Config.Groups {
		...
		if isMemberOf(memberOf, group.GroupDN) {
			...
			if extUser.IsGrafinsightAdmin == nil || !*extUser.IsGrafinsightAdmin {
				extUser.IsGrafinsightAdmin = group.IsGrafinsightAdmin
			}
		}
	}
...
func (server *Server) serializeUsers(
	...
	for _, user := range entries {
		extUser, err := server.buildGrafinsightUser(user)
		if err != nil {
			return nil, err
		}
