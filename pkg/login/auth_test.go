...
func TestAuthenticateUser(t *testing.T) {
	authScenario(t, "When a user authenticates without setting a password", func(sc *authScenarioContext) {
		...
		mockLoginUsingGrafinsightDB(nil, sc)
		...
		assert.False(t, sc.grafinsightLoginWasCalled)
	...
	authScenario(t, "When grafinsight user authenticate with valid credentials", func(sc *authScenarioContext) {
		mockLoginAttemptValidation(nil, sc)
		mockLoginUsingGrafinsightDB(nil, sc)
		...
		assert.True(t, sc.loginAttemptValidationWasCalled)
		assert.True(t, sc.grafinsightLoginWasCalled)
		assert.False(t, sc.ldapLoginWasCalled)
		assert.False(t, sc.saveInvalidLoginAttemptWasCalled)
		assert.Equal(t, "grafinsight", sc.loginUserQuery.AuthModule)
	})

	authScenario(t, "When grafinsight user authenticate and unexpected error occurs", func(sc *authScenarioContext) {
		customErr := errors.New("custom")
		mockLoginAttemptValidation(nil, sc)
		mockLoginUsingGrafinsightDB(customErr, sc)
		mockLoginUsingLDAP(true, ErrInvalidCredentials, sc)
		mockSaveInvalidLoginAttempt(sc)
	...
	authScenario(t, "When a non-existing grafinsight user authenticate and ldap disabled", func(sc *authScenarioContext) {
		mockLoginAttemptValidation(nil, sc)
		mockLoginUsingGrafinsightDB(models.ErrUserNotFound, sc)
		mockLoginUsingLDAP(false, nil, sc)
		mockSaveInvalidLoginAttempt(sc)
	...
	authScenario(t, "When a non-existing grafinsight user authenticate and invalid ldap credentials", func(sc *authScenarioContext) {
		mockLoginAttemptValidation(nil, sc)
		mockLoginUsingGrafinsightDB(models.ErrUserNotFound, sc)
	...
	authScenario(t, "When a non-existing grafinsight user authenticate and valid ldap credentials", func(sc *authScenarioContext) {
		mockLoginAttemptValidation(nil, sc)
		mockLoginUsingGrafinsightDB(models.ErrUserNotFound, sc)
		mockLoginUsingLDAP(true, nil, sc)
		mockSaveInvalidLoginAttempt(sc)
	...
	authScenario(t, "When a non-existing grafinsight user authenticate and ldap returns unexpected error", func(sc *authScenarioContext) {
		customErr := errors.New("custom")
		mockLoginAttemptValidation(nil, sc)
		mockLoginUsingGrafinsightDB(models.ErrUserNotFound, sc)
	...
	authScenario(t, "When grafinsight user authenticate with invalid credentials and invalid ldap credentials", func(sc *authScenarioContext) {
		mockLoginAttemptValidation(nil, sc)
		mockLoginUsingGrafinsightDB(ErrInvalidCredentials, sc)
		mockLoginUsingLDAP(true, ldap.ErrInvalidCredentials, sc)
		mockSaveInvalidLoginAttempt(sc)
...
type authScenarioContext struct {
	...
	grafinsightLoginWasCalled        bool
...
func mockLoginUsingGrafinsightDB(err error, sc *authScenarioContext) {
	loginUsingGrafinsightDB = func(query *models.LoginUserQuery) error {
		sc.grafinsightLoginWasCalled = true
		return err
	}
}
...
func authScenario(t *testing.T, desc string, fn authScenarioFunc) {
	...
	t.Run(desc, func(t *testing.T) {
		origLoginUsingGrafinsightDB := loginUsingGrafinsightDB
		...
		t.Cleanup(func() {
			loginUsingGrafinsightDB = origLoginUsingGrafinsightDB
