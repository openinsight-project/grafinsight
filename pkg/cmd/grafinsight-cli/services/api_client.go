...
type GrafinsightComClient struct {
	retryCount int
}

func (client *GrafinsightComClient) GetPlugin(pluginId, repoUrl string) (models.Plugin, error) {
...
func (client *GrafinsightComClient) DownloadFile(pluginName string, tmpFile *os.File, url string, checksum string) (err error) {
	...
	if len(checksum) > 0 && checksum != fmt.Sprintf("%x", h.Sum(nil)) {
		return fmt.Errorf("expected SHA256 checksum does not match the downloaded archive - please contact security@grafinsight.com")
	}
	return nil
}

func (client *GrafinsightComClient) ListAllPlugins(repoUrl string) (models.PluginRepo, error) {
...
func createRequest(repoUrl string, subPaths ...string) (*http.Request, error) {
	...
	req.Header.Set("grafinsight-version", grafinsightVersion)
	req.Header.Set("grafinsight-os", runtime.GOOS)
	req.Header.Set("grafinsight-arch", runtime.GOARCH)
	req.Header.Set("User-Agent", "grafinsight "+grafinsightVersion)
