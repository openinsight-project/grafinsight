...
// InstallPlugin downloads the plugin code as a zip file from the Grafinsight.com API
...
func InstallPlugin(pluginName, version string, c utils.CommandLine, client utils.ApiClient) error {
	...
	var checksum string
	if downloadURL == "" {
		if strings.HasPrefix(pluginName, "grafinsight-") {
			// At this point the plugin download is going through grafinsight.com API and thus the name is validated.
			// Checking for grafinsight prefix is how it is done there so no 3rd party plugin should have that prefix.
...
func extractFile(file *zip.File, filePath string) (err error) {
	...
	if err != nil {
		...
		if unwrappedError != nil && strings.EqualFold(unwrappedError.Error(), "text file busy") {
			return fmt.Errorf("file %q is in use - please stop Grafinsight, install the plugin and restart Grafinsight", filePath)
		}
